...
def parse_config(config_path):
    ...
    extensions = config.get(
        "Extensions", "extensions", fallback="mpg, ts, mp4, mov, mxf"
    )
	...
	#product = config.get("SVOD", "product", fallback="MOD").strip()
    product = config.get("SVOD", "product", fallback="SVOD").strip()
    if len(product) > 20:
    ...
    provider = config.get("SVOD", "provider", fallback="001").strip()
    if len(provider) > 20:
        raise ConfigurationError(
    ...
    ecn_2009 = config.get("SVOD", "ecn_2009", fallback="False")
